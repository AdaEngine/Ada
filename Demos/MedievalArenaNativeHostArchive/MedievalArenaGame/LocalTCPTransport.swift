...
public actor LocalTCPTransport: MultiplayerTransport {
    ...
    private let diagnostics: ArenaDiagnostics
    ...
    public init(host: String, port: UInt16, diagnostics: ArenaDiagnostics) {
        self.host = host
    ...
    public func start(configuration: MultiplayerTransportConfiguration) async throws {
        ...
        case .host:
            ...
            diagnostics.record("listening on 0.0.0.0:\(portNumber)")
        case .peer:
            let connection = NWConnection(host: NWEndpoint.Host(host), port: port, using: .tcp)
            hostConnection = connection
            configure(connection)
            connection.start(queue: .global(qos: .userInitiated))
            diagnostics.record("connecting to \(host):\(portNumber)")
        }
    }
    ...
    public func stop() async {
        ...
        configuration = nil
        diagnostics.record("transport stopped")
    }
    ...
    private func listenerChanged(_ state: NWListener.State) {
        ...
        case .failed(let error):
            ...
            diagnostics.record("listener failed: \(error)")
        case .ready:
            diagnostics.record("listener ready")
        default:
            break
        }
    }
    ...
    private func connectionChanged(_ connection: NWConnection, state: NWConnection.State) async {
        ...
        case .waiting(let error):
            diagnostics.record("connection waiting: \(error)")
        case .cancelled:
    ...
    private func receiveHeader(from connection: NWConnection) {
        connection.receive(minimumIncompleteLength: 4, maximumLength: 4) { [weak self, weak connection] data, _, isComplete, error in
            ...
            Task {
                ...
                let size = data.withUnsafeBytes { UInt32(bigEndian: $0.loadUnaligned(as: UInt32.self)) }
    ...
    private func receiveHello(_ hello: Hello, from connection: NWConnection) async throws {
        ...
        case .host:
            ...
            diagnostics.record("connected peer \(hello.peerID.rawValue.uuidString)")
        case .peer:
            ...
            continuation.yield(.connected(hello.peerID))
            diagnostics.record("connected host \(hello.peerID.rawValue.uuidString)")
        }
    }
    ...
    private func sendEnvelope(_ envelope: Data, through connection: NWConnection) async throws {
        var size = UInt32(envelope.count).bigEndian
    ...
    private func fail(_ connection: NWConnection, error: any Error) {
        diagnostics.record("connection failed: \(error)")
        continuation.yield(.failed(error.localizedDescription))
        connection.cancel()
        disconnect(connection)
    }
    ...
    private func disconnect(_ connection: NWConnection) {
        ...
        if let peer = peersByConnection.removeValue(forKey: identifier) {
            ...
            diagnostics.record("disconnected peer \(peer.rawValue.uuidString)")
        } else if connection === hostConnection, let hostPeerID {
            self.hostPeerID = nil
            hostConnection = nil
            continuation.yield(.disconnected(hostPeerID))
            diagnostics.record("disconnected host \(hostPeerID.rawValue.uuidString)")
        }
    }
}
