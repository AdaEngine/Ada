...
fn main_1() {
  var vertexIndex : u32;
  var uv : vec2f;
  vertexIndex = bitcast<u32>(gl_VertexIndex);
  uv = (vec2f(f32((vertexIndex >> 1u)), f32((vertexIndex & 1u))) * 2.0f);
...
@vertex
fn ssr_vertex(@builtin(vertex_index) gl_VertexIndex_param : u32) -> main_out {
